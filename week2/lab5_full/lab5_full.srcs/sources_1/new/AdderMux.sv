module AdderMux(input logic [15:0] IR, PC, SR1OUT,
    ...
    Mux4to1Block mux_addr2(.d0(1'b0), .d1({16{IR[5:0]}}), .d2({16{IR[8:0]}}), .d3({16{IR[10:0]}}), .select(ADDR2MUX), .mux_data(addr2_out));
    Mux2to1Block mux_addr1(.d0(SR1OUT), .d1(PC), .select(ADDR1MUX), .mux_data(addr1_out));
